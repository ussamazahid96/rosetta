...
VERILOG_SRC_DIR := $(TOP)/src/main/resources
APP_SRC_DIR := $(TOP)/src/main/python
VIVADO_PROJ_SCRIPT := $(TOP)/src/main/scripts/$(NAME)/make-vivado-project.tcl
VIVADO_SYNTH_SCRIPT := $(TOP)/src/main/scripts/synth-vivado-project.tcl
...
# copy bitfile to the deployment folder, make hwh script for bitfile loader
pynq_hw: bitfile
	mkdir -p $(BUILD_DIR_PYNQ); cp $(GEN_BITFILE_PATH) $(BUILD_DIR_PYNQ)/rosetta.bit; cp $(HWH_FILE) $(BUILD_DIR_PYNQ)/rosetta.hwh
