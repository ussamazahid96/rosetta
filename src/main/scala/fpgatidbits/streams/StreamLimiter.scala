...
object StreamLimiter {
  def apply(in: AXIStreamMasterIF[UInt], start: Bool, count: UInt): DecoupledIO[UInt] = {
    val limiter = Module(new StreamLimiter(in.TDATA.getWidth)).io
    limiter.start := start
    limiter.byteCount := count
    limiter.streamIn.bits := in.TDATA
    limiter.streamIn.valid := in.TVALID
    in.TREADY := limiter.streamIn.ready
    limiter.streamOut
  }
}
