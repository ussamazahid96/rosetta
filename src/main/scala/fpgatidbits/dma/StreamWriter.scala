...
override def cloneType = { new StreamWriterIF(w, p).asInstanceOf[this.type] }
}
...
class StreamWriter(val p: StreamWriterParams) extends Module {
  ...
  if(p.streamWidth == p.mem.dataWidth) 
  {
    io.in <> io.wdat
  }
  else if(p.streamWidth > p.mem.dataWidth) 
  {
    val str_dwnszr_out = StreamDownsizer(io.in, p.mem.dataWidth, clock, reset)
    io.wdat.bits := str_dwnszr_out.TDATA
    io.wdat.valid := str_dwnszr_out.TVALID
    str_dwnszr_out.TREADY := io.wdat.ready    
  } 
  else 
  {
    val str_upszr_out = StreamUpsizer(io.in, p.mem.dataWidth)
    io.wdat.bits := str_upszr_out.TDATA
    io.wdat.valid := str_upszr_out.TVALID
    str_upszr_out.TREADY := io.wdat.ready
  }
}
